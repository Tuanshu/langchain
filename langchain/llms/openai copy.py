...
def _update_response(response: Dict[str, Any], stream_response: Dict[str, Any]) -> None:
    ...
    response["choices"][0]["logprobs"] = stream_response["choices"][0]["logprobs"] if 'logprobs' in stream_response["choices"][0] else None
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                for stream_resp in completion_with_retry(
                    ...
                    if run_manager:
                        # ts disable logprobs with else... condtion, since the textgeneration api does not return this
                        run_manager.on_llm_new_token(
                            stream_resp["choices"][0]["text"],
                            verbose=self.verbose,
                            logprobs=stream_resp["choices"][0]["logprobs"] if 'logprobs' in stream_resp["choices"][0] else None,
                        )
        ...
        for _prompts in sub_prompts:
            if self.streaming:
                ...
                async for stream_resp in await acompletion_with_retry(
                    ...
                    if run_manager:
                        # ts try handle logprobs not exist
